...
[ROADMAP.md](ROADMAP.md) gives the milestones: the Counter on a chain (M1), a minimal token (M2), laws for any state (M3) and the rest of ERC-20 (M4), all done.
...
- Supported now: `sload`, `sstore`, mappings (`load(slot, key)`, `store(slot, key, value)`) and nested mappings (`load2(slot, outer, inner)`, `store2(slot, outer, inner, value)`), `caller`, checked `add` and `sub`, `require`, `log`, `pure`, Nat and address parameters, and literal constants. The reader rejects everything else.
