...
This is a small consumer of the shared [Core.Program](../vendor/bend-frontend/src/core.bend). The frontend still accepts full upstream Bend; this backend accepts U32 arguments/results, variables, local bindings, and direct Base U32 addition/multiplication. Other runtime constructs are rejected.
...
bun run check
mkdir -p build
bun vendor/bend-frontend/host/run.js yul-u32/emit.bend > build/calculate.yul
solc --strict-assembly --evm-version shanghai --bin build/calculate.yul
bun run test

# Select another checked function:
bun vendor/bend-frontend/host/run.js yul-u32/emit.bend yul-u32/PROOF.bend program.scopes
```
...
See [Connecting Bend to backends](https://github.com/ind-igo/bend-frontend/blob/main/docs/backends.md) for the extension process and target-specific decisions.
